...
// var events = cal.getEvents(new Date("January 3, 2014 00:00:00 CST"), new Date("January 14, 2014 23:59:59 CST"), {search: 'apollo test'});
// 
...
//
  ...
  var events = cal.getEvents(new Date("October 1, 2015 00:00:00 CST"), new Date("September 1, 2016 23:59:59 CST"), {search: 'test schema'});
...
//cell.setFormula('=DAY(F' +row+ ')*24+(HOUR(F' +row+ ')+(MINUTE(F' +row+ ')/60))-(DAY(E' +row+ ')*24+HOUR(E' +row+ ')+(MINUTE(E' +row+ ')/60))');
//cell.setNumberFormat('.00');

var cell=sheet.getRange(row,7);
cell.setFormula('=days360(E' +row+ ',F' +row+ ',)');
cell.setNumberFormat('.00');
